        ...
        int n_cells)
{
    ...
    int i, ibox, iatom;
    printf("System state:\n");
    for (ibox=0;ibox<n_cells;ibox++) {
        for (iatom=0;iatom<sim_H.grid.n_atoms[ibox];iatom++) {

            i = ibox*MAXATOMS + iatom;

            printf("%02d, %02d, X=(%+020.12e %+020.12e %+020.12e) 1 P=(%+020.12e %+020.12e %+020.12e) F=(%+020.12e %+020.12e %+020.12e)\n",
                    ibox, iatom, 
...
void CreateDevGrid(dev_grid_t *grid_D, int n_r_size, int n_nl_size, int n_n_size) 
{
    ...
    oclCreateReadWriteBuffer(&grid_D->bounds, sizeof(real_t)*3);
}
...
void PutGrid(host_grid_t grid_H, dev_grid_t grid_D)
{
    ...
    oclCopyToDevice(grid_H.bounds, grid_D.bounds, sizeof(real_t)*3);
}
...
void PutEamPot(host_eam_pot_t eam_pot_H, dev_eam_pot_t eam_pot_D)
{
    ...
    oclCopyToDevice(eam_pot_H.n_values, eam_pot_D.n_values, sizeof(real_t)*3);
}


void oclRunKernel(cl_kernel kernel, cl_event *event, size_t* n_global, size_t* n_local)
{
    int err = clEnqueueNDRangeKernel(commandq, kernel, 2, NULL, n_global, NULL, 0, NULL, event);
    if (err != CL_SUCCESS)
    {
...
{ 
    ...
    err |= clSetKernelArg(force_kernels[2], n_arg++, sizeof(cl_mem), &sim_D.e);
        ...
        )
{
    ...
    grid_H->bounds = malloc(3*sizeof(real_t));
...
void HostEAMInit(host_eam_pot_t *eam_pot_H, simflat_t *sim) 
{
    ...
    eam_pot_H->n_p_rho_size = (3 + new_pot->rho->n)*sizeof(real_t);
    printf("rho potential size = %d\n", eam_pot_H->n_p_rho_size);

    n_v_phi = new_pot->phi->n;
    eam_pot_H->n_p_phi_size = (3 + new_pot->phi->n)*sizeof(real_t);
    printf("phi potential size = %d\n", eam_pot_H->n_p_phi_size);

    n_v_F = new_pot->f->n;
    eam_pot_H->n_p_F_size = (3 + new_pot->f->n)*sizeof(real_t);
    ...
    eam_pot_H->rho[n_v_rho+0] = new_pot->rho->x0;
    eam_pot_H->rho[n_v_rho+1] = new_pot->rho->xn;
    eam_pot_H->rho[n_v_rho+2] = new_pot->rho->invDx;

    for (i=0;i<n_v_rho;i++)
    {
        eam_pot_H->rho[i] = new_pot->rho->values[i];
    }

    eam_pot_H->phi[n_v_phi+0] = new_pot->phi->x0;
    eam_pot_H->phi[n_v_phi+1] = new_pot->phi->xn;
    eam_pot_H->phi[n_v_phi+2] = new_pot->phi->invDx;

    for (i=0;i<n_v_phi;i++)
    {
        eam_pot_H->phi[i] = new_pot->phi->values[i];
    }

    eam_pot_H->F[n_v_F+0] = new_pot->f->x0;
    eam_pot_H->F[n_v_F+1] = new_pot->f->xn;
    eam_pot_H->F[n_v_F+2] = new_pot->f->invDx;

    for (i=0;i<n_v_F;i++)
    {
        eam_pot_H->F[i] = new_pot->f->values[i];
    }
...
void HostLJInit(host_lj_pot_t *lj_pot_H, simflat_t *sim)
{
...
void HostSimInit (host_sim_t *sim_H, simflat_t *sim)
{
    ...
    sim_H->rmass = (real_t)((double)1822.83);
    ...
    sim_H->grid.bounds = malloc(3*sizeof(real_t));
...
void DevSimInit(dev_sim_t *sim_D, host_sim_t sim_H)
{
    ...
    if (sim_H.eam_flag){
        ...
        oclCreateReadWriteBuffer(&sim_D->eam_pot.n_values, sizeof(real_t)*3);
...
void computeForceOCL(cl_kernel* force_kernels, cl_event Force_event, size_t* n_global, size_t* n_local, int eam_flag)
{
    /** Execute the appropriate force kernels **/

    int err;
    real_t t_simple, t_overall;
    real_t t_total = 0.0;
    if (eam_flag) {
        if (DIAG_LEVEL > 1) {
        ...
        t_total += t_simple;
        if (DIAG_LEVEL > 0)
        printf("Kernel EAM_Force executed in %.3e secs.\n", t_total);
    } else {
        if (DIAG_LEVEL > 1) {
        ...
        GetElapsedTime(Force_event, &t_simple, &t_overall);
        if (DIAG_LEVEL > 0)
        printf("Kernel LJ_Force executed in %.3e secs.\n", t_simple);
    }

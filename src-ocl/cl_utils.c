...
int oclInit(int gpu_request)
{
    ...
    else 
    {
        ...
        for (i = 0; i < num_platforms; i++) {
            err = clGetPlatformIDs(num_platforms, &platform_id[i], NULL);
            if (err != CL_SUCCESS)
            {
    ...
    num_cpus = 0;
    err = clGetDeviceIDs(platform_id[0], CL_DEVICE_TYPE_CPU, 0, NULL, &num_cpus);
    if (err != CL_SUCCESS)
    {
    ...
    num_gpus = 0;
    err = clGetDeviceIDs(platform_id[0], CL_DEVICE_TYPE_GPU, 0, NULL, &num_gpus);
    if (err != CL_SUCCESS)
    {
    ...
    int usingGPU = 0;
    if (gpu_request) { // branch for request for GPU
        if (num_gpus > 0) {

            printf("Connecting to GPU as compute device\n");
            usingGPU = 1;
            err = clGetDeviceIDs(platform_id[0], CL_DEVICE_TYPE_GPU, 1, &device_id, NULL);
            if (err != CL_SUCCESS)
            {
            ...
            if (num_cpus > 0 ) {
                ...
                err = clGetDeviceIDs(platform_id[0], CL_DEVICE_TYPE_CPU, 1, &device_id, NULL);
                if (err != CL_SUCCESS)
                {
                    printf("Failed to create a device group!\n");
                    return EXIT_FAILURE;
    ...
    } else { // branch for no GPU request
        if (num_cpus > 0 ) {
    ...
    err = clGetDeviceInfo(device_id , CL_DEVICE_NAME, 80*sizeof(char), &device_name, NULL);
    ...
    else context = clCreateContext(cpsGL, 1, &device_id, NULL, NULL, &err);
#else
    GLXContext gGlCtx = glXGetCurrentContext();
    cl_context_properties cpsGL[] = { CL_CONTEXT_PLATFORM, (cl_context_properties)platform_id[0],
                                      CL_GLX_DISPLAY_KHR, (intptr_t) glXGetCurrentDisplay(),
                                      CL_GL_CONTEXT_KHR, (intptr_t) gGlCtx, 0};
    context = clCreateContext(cpsGL, 1, &device_id, NULL, NULL, &err);
#endif
    ...
    commandq = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
    if (!commandq)
    {
...
void oclCopyToDevice(void* host_buffer, cl_mem device_buffer, int size)
{
    int err = clEnqueueWriteBuffer(commandq, device_buffer, CL_TRUE, 0, size, host_buffer, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
...
void oclCopyToHost(cl_mem device_buffer, void* host_buffer, int size)
{
    int err = clEnqueueReadBuffer(commandq, device_buffer, CL_TRUE, 0, size, host_buffer, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        ...
        cl_command_queue* commandq)
{
    ...
    err = clGetDeviceIDs(platform_id[0], GPU ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU, 0, NULL, &num_devices);
    if (err != CL_SUCCESS)
    {
    ...
    err = clGetDeviceIDs(platform_id[0], GPU ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU, 1, device_id, NULL);
    if (err != CL_SUCCESS)
    {
        printf("Failed to create a device group!\n");
        return EXIT_FAILURE;

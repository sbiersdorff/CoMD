"""
usubs: interactive python  utility routines
author: Sriram Swaminarayan

usubs is a set of utility routines for getting user input.  Typically
the user is given a command and the results are interpreted on a
word-by word basis. Strings enclosed in double or single quotes are
interpreted as literals.

Remember, more than a set of subroutines, usubs is an input philosophy.
To really buy into it, you must do _ALL_ of your input through usubs.
Mixing usubs calls with regular input calls can have disastrous
results.
...
class input:
    ...
    def __init__(self, inBuffer=None, noNewInput=0, debug=0):
        ...
        if(self.debug): print self.buffer
